...
- Dimension Parts: `X`、`Y`、`Z`、`Largest`
- 各Partの`Separator After`
- 小数桁数: 0から4
- 丸め: 四捨五入の`Round`、切り捨ての`Floor`、切り上げの`Ceil`
- 単位サフィックス
- 軸ラベル
- 末尾の0削除

Dimension Partは追加、削除、並べ替えができます。初期状態は`X("x")`、`Y("x")`、`Z("")`です。Zだけ必要な場合はZ以外を削除し、最大寸法だけ必要な場合は`Largest`を使用します。

`Largest`で複数軸の寸法が同じ場合はX、Y、Zの順で選びます。軸ラベルを有効にすると、`Largest`という文字ではなく実際に選ばれた軸名を出力します。

Dimension Partが0件の場合はPreviewとApplyが`Empty Name`になり、プリセットへも保存できません。旧バージョンのSceneとプリセットにある軸順・軸区切り設定は、最初の読込時にDimension Partsへ自動変換されます。
