...
3. `Name Mode` で、Object名を使うかMeshデータ名を使うか選びます。
4. `Collection Color` で、色を変更しない場合は `Keep`、色を消す場合は `None`、色を付ける場合は色番号ボタンを選びます。
...
- `Collection Color` で色を選んだ場合、作成または再利用した移動先コレクションのOutliner色が指定色に更新されます。
- `Name Mode` が `Mesh Name` の場合、MeshオブジェクトはMeshデータ名を使います。Mesh以外のオブジェクトはObject名を使います。
